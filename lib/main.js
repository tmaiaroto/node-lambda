...
Lambda.prototype.run = function (program) {
  ...
  // Accept this from the .env file too (CLI argument didn't seem to work anyway).
  if (process.env.CONFIG_FILE) {
    program.configFile = process.env.CONFIG_FILE;
  }
  if (program.configFile) {
    this._setEnvironmentVars(program, "./");
  }
...
Lambda.prototype._rsync = function (program, codeDirectory, callback) {
  ...
  // Just exclude the main .env file. We want to include other config files like deploy.env. Those will be needed by the Lambda
  // and are intended to be included. However, .env is for configuring the Lambda and that's not needed.
  // Ultimately we should not send any of these files to the Lambda and instead should use a distributed configuration. DynamoDB, etc.
  exec('rsync -r --exclude=.git --exclude=.env --exclude=*.log --exclude=node_modules . ' + codeDirectory, function (err) {
    if (err) {
      throw err;
    }
    ...
    _this._npmInstall(program, codeDirectory, function (err) {
      ...
      archive(program, codeDirectory, function (err, buffer) {
        ...
        async.map(regions, function (region, cb) {
          ...
          // Region is a different concern. Handle it independently so it's not tied to credentials.
          if(region) {
